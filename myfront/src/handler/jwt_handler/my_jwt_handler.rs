...
use crate::auth::{AuthError, Claims, get_auth_user, JWT};
use crate::global_request_id::get_request_id;
use crate::handler::AuthInfo;

/// 使用用户名&密码获取 JWT Token
#[debug_handler]
pub async fn get_jwt_token(Json(AuthInfo{ client_id, client_pwd }): Json<AuthInfo>) -> Result<Json<serde_json::Value>, AuthError> {

    let user = match get_auth_user(&client_id, &client_pwd) {
        Some(user) => user,
    ...
    let claims = JWT.new_claims(client_id, client_name.to_string(), 30);
