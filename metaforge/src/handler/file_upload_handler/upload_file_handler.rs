...
pub async fn upload_file() -> Result<Html<String>, String> {
    let html = UploadFileTemplate
        .render()
        .map_err(|err| format!("upload_file 模板渲染失败：{}", err))?;
    Ok(Html(html))
}
...
pub async fn upload_file_action(
        ...
        .map_err(|err| err.to_string())?
    {
        let filename = format!(
            ...
            file.file_name().ok_or("获取文件名称出错。")?
        ); // 上传的文件名
        let upload_path = Path::new(&upload_path).join(&filename); //
        let data = file
            .bytes()
            .await
            .map_err(|err| format!("获取上传文件内容失败：{}", err))?; // 上传的文件的内容
...
async fn cn(msg: String) -> Result<(HeaderMap, String), String> {
    ...
    headers.insert(
        ...
        "text/plain;charset=utf-8"
            .parse()
            .map_err(|err| format!("设置 Content-Type 失败：{}", err))?,
    );
    Ok((headers, msg))
}
