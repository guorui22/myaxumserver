...
async fn test_find_square() {
    let mut client: CalculatorServiceClient<InterceptedService<Channel, MyInterceptor>> = CalculatorServiceClient(TEST_JWT.into()).await.unwrap();
...
generate_function_for_grpc_client!(CalculatorServiceClient, TEST_ADDRESS);
